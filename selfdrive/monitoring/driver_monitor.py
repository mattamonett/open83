...
_AWARENESS_TIME = 3600.  # passive wheel touch total timeout
_AWARENESS_PRE_TIME_TILL_TERMINAL = 150.
...
# people don't seem to look straight when they drive relaxed, rather a bit up
_PITCH_NATURAL_OFFSET = 0.02
# people don't seem to look straight when they drive relaxed, rather a bit to the right (center of car)
_YAW_NATURAL_OFFSET = 0.08

_HI_STD_TIMEOUT = 5
# fall back to wheel touch if model is uncertain for a long time
_HI_STD_FALLBACK_TIME = 10
_DISTRACTED_FILTER_TS = 0.25  # 0.6Hz

_POSE_CALIB_MIN_SPEED = 13  # 30 mph
# valid data counts before calibration completes, 1 seg is 600 counts
_POSE_OFFSET_MIN_COUNT = 600
# stop deweighting new data after 6 min, aka "short term memory"
_POSE_OFFSET_MAX_COUNT = 3600
...
def face_orientation_from_net(angles_desc, pos_desc, rpy_calib, is_rhd):
    ...
    face_pixel_position = (
        (pos_desc[0] + .5)*W - W + FULL_W, (pos_desc[1]+.5)*H)
...
class DriverPose():
    def __init__(self):
        ...
        self.pitch_std = 0.
        self.roll_std = 0.
        self.pitch_offseter = RunningStatFilter(
            max_trackable=_POSE_OFFSET_MAX_COUNT)
        self.yaw_offseter = RunningStatFilter(
            max_trackable=_POSE_OFFSET_MAX_COUNT)
        self.low_std = True
        self.cfactor = 1.
...
class DriverStatus():
    ...
    def set_policy(self, model_data):
        ...
        self.pose.cfactor = interp(ep, [0, 0.5, 1], [
                                   _METRIC_THRESHOLD_STRICT, _METRIC_THRESHOLD, _METRIC_THRESHOLD_SLACK])/_METRIC_THRESHOLD
        self.blink.cfactor = interp(ep, [0, 0.5, 1], [
                                    _BLINK_THRESHOLD_STRICT, _BLINK_THRESHOLD, _BLINK_THRESHOLD_SLACK])/_BLINK_THRESHOLD
    ...
    def get_pose(self, driver_state, cal_rpy, car_speed, op_engaged):
        ...
        self.pose.roll, self.pose.pitch, self.pose.yaw = face_orientation_from_net(
            driver_state.faceOrientation, driver_state.facePosition, cal_rpy, self.is_rhd_region)
        ...
        self.blink.left_blink = driver_state.leftBlinkProb * \
            (driver_state.leftEyeProb > _EYE_THRESHOLD) * \
            (driver_state.sunglassesProb < _SG_THRESHOLD)
        self.blink.right_blink = driver_state.rightBlinkProb * \
            (driver_state.rightEyeProb > _EYE_THRESHOLD) * \
            (driver_state.sunglassesProb < _SG_THRESHOLD)
    ...
    def update(self, events, driver_engaged, ctrl_active, standstill):
        ...
        if self.face_detected and self.hi_stds * DT_DMON > _HI_STD_TIMEOUT and self.hi_std_alert_enabled:
            ...
            # only showed once until orange prompt resets it
            self.hi_std_alert_enabled = False

        if (driver_attentive and self.face_detected and self.pose.low_std and self.awareness > 0):
            # only restore awareness when paying attention and alert is not red
            self.awareness = min(self.awareness + ((_RECOVERY_FACTOR_MAX-_RECOVERY_FACTOR_MIN)*(
                1.-self.awareness)+_RECOVERY_FACTOR_MIN)*self.step_change, 1.)
            if self.awareness == 1.:
